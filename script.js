...
function getShoeList() {
  axios
      ...
      .then((response) => {
          ...
          shoes.forEach((shoe) => {
              shoeList += `
                <div class="shoe-item col-4 mb-4">
                  <div class="card">
              ...
              <div class="col-8">
                ...
                <div class="d-flex align-items-center">
                  ...
                  <div id="sizeOptions" class="d-flex flex-wrap">
                    ${shoe.size.map(size => `<button class="btn btn-outline-secondary m-1" onclick="selectSize('${size}')">${size}</button>`).join('')}
                  </div>
                </div>
...
function selectSize(size) {
  ...
  document.querySelector(`button:contains('${size}')`).classList.add('active');
